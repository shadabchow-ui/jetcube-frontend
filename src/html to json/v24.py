...
def _amazon_title_signal_blob(original_title: str) -> str:
  """Turn the raw Amazon title into a low-noise text blob (signal only)."""
  t = clean_ws(original_title or "")
  if not t:
    return ""
  # drop years + heavy punctuation
  ...
  if toks:
    first = toks[0]
    # strip a leading brand-ish token (common in Amazon titles)
    if (first.isupper() and len(first) >= 3) or re.match(r"^[A-Z0-9]{3,}$", first):
      toks = toks[1:]
    elif first[0].isupper() and first[1:].islower() and first.lower() not in ("womens","mens","girls","boys","kids"):
      # sometimes Brand is TitleCase; keep common gender words
      toks = toks[1:]
  t = " ".join(toks)

  # normalize women's forms so downstream regexes hit consistently
  t = t.replace("Women'S", "Womens").replace("Women's", "Womens").replace("women's", "Womens").replace("Women’s", "Womens").replace("women’s", "Womens")
  t = t.replace("Men'S", "Mens").replace("Men's", "Mens").replace("men's", "Mens").replace("Men’s", "Mens").replace("men’s", "Mens")
  return clean_ws(t).lower()
...
def build_seo_title_v5(seed: str, category: str, bullets: List[str], specs: Dict[str,str], original_title: str = "") -> str:
  """
  Build a clean, SEO-safe title.
  Inputs: category breadcrumb + bullets + specs + (optional) original title used ONLY as extra keyword signal.
  Output: regex-safe, <= ~120 chars.
  """
  title_blob = _amazon_title_signal_blob(original_title)
  blob = _norm_text_blob([
    category,
    title_blob,
    " ".join(bullets or []),
    " ".join([f"{k} {v}" for k,v in (specs or {}).items()]),
  ])
  ...
  if re.search(r"\bmen\b|\bmens\b|\bman\b", blob):
    ...
    if re.search(r"\bwomen\b|\bwomens\b|\bladies\b|\bfemale\b", blob):
      gender = "Womens"
    elif any(k in blob for k in ["dress","pants","socks","shirt","sweater","hoodie","jacket","coat","skirt","jumpsuit","legging","tights","cardigan"]):
      gender = "Womens"

  # core item type (noun)
  core = ""
  if any(k in blob for k in ["pants","trousers","slacks","jogger","joggers","legging","leggings","tights"]):
    core = "Pants" if "pants" in blob or "trouser" in blob or "slack" in blob else "Leggings"
  elif any(k in blob for k in ["dress","gown"]):
    core = "Dress"
  elif any(k in blob for k in ["sweater","cardigan","pullover","hoodie","sweatshirt"]):
    if "cardigan" in blob:
      core = "Cardigan"
    elif "hoodie" in blob:
      core = "Hoodie"
    elif "sweatshirt" in blob:
      core = "Sweatshirt"
    else:
      core = "Sweater"
  elif any(k in blob for k in ["sock","socks"]):
    core = "Socks"
    ...
    core = "Skirt"
  elif any(k in blob for k in ["jacket","coat","parka","windbreaker"]):
    core = "Jacket" if "jacket" in blob or "windbreaker" in blob else "Coat"
  elif any(k in blob for k in ["bra","bralette","sports bra"]):
    core = "Bra"
    ...
    # fallback to the last breadcrumb node if it looks usable
    parts = parse_category_path(category)
    leaf = parts[-1] if parts else ""
    leaf_low = (leaf or "").lower()
    if leaf_low:
      if "dress" in leaf_low:
        core = "Dress"
      elif "pant" in leaf_low:
        core = "Pants"
      elif "sock" in leaf_low:
        core = "Socks"
      elif "sweater" in leaf_low or "cardigan" in leaf_low:
        core = "Sweater"
      else:
        core = leaf.title().strip() or "Item"
    else:
      core = "Item"

  # attributes / descriptors (pull from both page text and original title signals)
  attrs: List[str] = []

  # one deterministic descriptor to avoid overly-generic titles
  desc = infer_descriptor_from_text(seed, f"{category} {original_title}", bullets, specs)
  if desc:
    attrs.append(desc)

  # high-signal attribute phrases (ordered by typical shopper importance)
  pairs = [
    ("wide leg", "Wide Leg"),
    ("straight leg", "Straight Leg"),
    ("flare", "Flare Leg"),
  ...
  # remove duplicates while preserving order + keep it short
  attrs = uniq_keep_order(attrs)
  # avoid repeating core word as an attribute
  core_low = core.lower()
  attrs = [a for a in attrs if a.lower() not in (core_low, core_low+"s")]
  attrs = attrs[:8]
  ...
  if not title or title.lower() in ("product", "item"):
...
def main(argv: List[str]) -> int:
  ...
  for fp in html_files:
    ...
    rewritten_title = build_seo_title_v5(ex.asin, ex.category, ex.bullets, ex.specs, ex.title)
