...
// 🔎 Change this string whenever you redeploy to prove the new function is live.
const PDP_BUILD = "asin-map-resolver-v2";
...
async function loadAsinMap(bucket: R2Bucket): Promise<{ map: Record<string, string>; key: string } | null> {
  ...
  // These match your bucket layout: jetcube-products/indexes/*
  const candidates = [
...
async function resolveAsinToHandle(
  bucket: R2Bucket,
  asin: string
): Promise<{ handle: string; key: string; mode: "map" | "alias" } | null> {
  const a = asin.trim().toUpperCase();

  // 1) Preferred: global map lookup (generated by converter)
  ...
  // 2) Fallback: per-ASIN alias objects (not ideal at scale, but harmless)
  const tried = [
    `asins/${a}.json`,
    `asin/${a}.json`,
    `asins/${a}.json.gz`,
...
export const onRequest: PagesFunction<Env> = async ({ params, env }) => {
  ...
  if (looksLikeAsin(slug)) {
    ...
    if (asinResolved?.handle) slug = asinResolved.handle;
  }
